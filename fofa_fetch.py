...
with open("ip.txt", "w", encoding="utf-8") as f:
    for ip_port in sorted(all_ips):
        f.write(ip_port + "\n")
print(f"ip.txt 已生成，{len(all_ips)} 个 IP:PORT")

# 2️⃣ 判断运营商
def get_isp(ip):
    # 电信
    if re.match(r"^(1(3[3-9]|47|53|7[37]|8[019])|36|37|39|42|43|45|49|58|59|60|61|100|101|102|103|104|105|106|107|108|109|110|111|112|113|114|115|116|117|118|119|120|121|122|123|124|125|126|127|175|176|177|178|180|181|182|183|184|185|186|187|188|189|199|223)\.", ip):
        return "电信"
    # 联通
...
async def fetch_city(session, ip_port):
    if ':' in ip_port:
    ...
    city = "未知"
    try:
        async with session.get(f"http://ip-api.com/json/{ip}?lang=zh-CN", timeout=10) as resp:
...
async def main():
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_city(session, ip_port) for ip_port in all_ips]
        await asyncio.gather(*tasks)

asyncio.run(main())

# 4️⃣ 保存按城市+运营商分类的文件（覆盖写入）
