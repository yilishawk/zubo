...
from flask import Flask, send_file, make_response, Response, request  # 新增request
...
CLEAN_INTERVAL = 43200  # 7200→43200（12小时清理一次）
...
def load_channel_config():
    ...
    default_config = {
        ...
        "中国教育1台", "中国教育2台", "中国教育3台", "中国教育4台", "早期教育"],
            "卫视频道": ["CHC动作电影", "CHC家庭影院", "CHC影迷电影", "淘电影", "淘精彩", "淘剧场", "淘4K", "淘娱乐", "淘BABY", "淘萌宠", "重温经典",
    ...
    try:
        ...
        with open(CHANNEL_CONFIG_FILE, "r", encoding="utf-8") as f:
            config = json.load(f)
        # 兼容前端提交的格式（前端是数组，转成字典）
        if isinstance(config.get("all_configs"), list):
            new_categories = {}
            new_mapping = default_config["mapping"]  # 保留默认映射
            for item in config["all_configs"]:
                new_categories[item["category"]] = item["channels"]
            config = {
...
def save_channel_config(new_config):
    ...
    try:
        # 兼容前端提交的格式
        if isinstance(new_config.get("all_configs"), list):
            categories = {}
            for item in new_config["all_configs"]:
                categories[item["category"]] = item["channels"]
            # 保留原有映射（或扩展映射逻辑）
            old_config = load_channel_config()
            new_config = {
                "categories": categories,
                "mapping": old_config["mapping"]
            }
        # 写入文件
        with open(CHANNEL_CONFIG_FILE, "w", encoding="utf-8") as f:
...
def get_config():
    ...
    # 适配前端格式：转成数组
    all_configs = []
    for category, channels in config["categories"].items():
        all_configs.append({
            "category": category,
            "channels": channels
        })
