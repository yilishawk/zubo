...
from flask import Flask, send_from_directory, make_response, Response, request
...
def update_config():
    try:
        ...
        if success:
            def regenerate_task():
                if not REGENERATE_LOCK.acquire(blocking=False):
                    return
                try:
                    asyncio.run(generate_itvlist())
                finally:
                    REGENERATE_LOCK.release()
            threading.Thread(target=regenerate_task, daemon=True).start()
        return {"code": 200 if success else 500, "msg": msg}
    except Exception as e:
        return {"code": 500, "msg": f"配置解析失败：{str(e)}"}
